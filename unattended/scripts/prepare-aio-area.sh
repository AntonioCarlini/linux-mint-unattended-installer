...
#
# $1 if not 0, the repo is already present and should not be updated
# $2 if not 0, rather than clone the repo, simply update it (i.e. pull)
prepare_unattended_install_repo()
{
    if [ "$1" != "0" ]; then
	if [ "$2" == "0" ]; then
	    echo "git clone"; exit 3
	    if ! git clone "${uai_repo}" "${uai_area}"; then
		echo "Failed to clone the repo to ${uai_area}"
		exit 1
	    fi
	else
	    echo "git pull"; exit 3
	    if ! git pull; then
